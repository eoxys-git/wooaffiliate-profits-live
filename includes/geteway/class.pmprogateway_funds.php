<?php
	...
	add_action('init', array('PMProGateway_funds', 'init'));

	/**
	 * PMProGateway_gatewayname Class
	 *
	 * Handles funds integration.
	 *
	 */
	class PMProGateway_funds extends PMProGateway
	{
		...
		{
			//make sure funds is a gateway option
			add_filter('pmpro_gateways', array('PMProGateway_funds', 'pmpro_gateways'));

			//add fields to payment settings
			add_filter('pmpro_payment_options', array('PMProGateway_funds', 'pmpro_payment_options'));
			add_filter('pmpro_payment_option_fields', array('PMProGateway_funds', 'pmpro_payment_option_fields'), 10, 2);

			//add some fields to edit user page (Updates)
			add_action('pmpro_after_membership_level_profile_fields', array('PMProGateway_funds', 'user_profile_fields'));
			add_action('profile_update', array('PMProGateway_funds', 'user_profile_fields_save'));

			//updates cron
			add_action('pmpro_activation', array('PMProGateway_funds', 'pmpro_activation'));
			add_action('pmpro_deactivation', array('PMProGateway_funds', 'pmpro_deactivation'));
			add_action('pmpro_cron_funds_subscription_updates', array('PMProGateway_funds', 'pmpro_cron_funds_subscription_updates'));

			//code to add at checkout if funds is the current gateway
			$gateway = pmpro_getOption("gateway");
			// echo $gateway;
			// exit;
			if($gateway == "funds")
			{
				add_action('pmpro_checkout_preheader', array('PMProGateway_funds', 'pmpro_checkout_preheader'));
				add_filter('pmpro_checkout_order', array('PMProGateway_funds', 'pmpro_checkout_order'));
				add_filter('pmpro_include_billing_address_fields', array('PMProGateway_funds', 'pmpro_include_billing_address_fields'));
				add_filter('pmpro_include_cardtype_field', array('PMProGateway_funds', 'pmpro_include_billing_address_fields'));
				add_filter('pmpro_include_payment_information_fields', array('PMProGateway_funds', 'pmpro_include_payment_information_fields'));
			}
		}

		/**
		 * Make sure funds is in the gateways list
		 *
		 * @since 1.8
		 */
		static function pmpro_gateways($gateways)
		{
			if(empty($gateways['funds']))
				$gateways['funds'] = __('funds', 'pmpro');

			return $gateways;
		}

		/**
		 * Get a list of payment options that the funds gateway needs/supports.
		 *
		 * @since 1.8
		 */
		...
		{
			//get funds options
			$funds_options = PMProGateway_funds::getGatewayOptions();

			//merge with others.
			$options = array_merge($funds_options, $options);

			return $options;
		}

		/**
		 * Display fields for funds options.
		 *
		 * @since 1.8
		 */
		...
		{
		?>
		<tr class="pmpro_settings_divider gateway gateway_funds" <?php if($gateway != "funds") { ?>style="display: none;"<?php } ?>>
			<td colspan="2">
				<?php _e('Funds Settings', 'pmpro'); ?>
			</td>
		</tr>
		<tr class="gateway gateway_funds" <?php if($gateway != "funds") { ?>style="display: none;"<?php } ?>>
			<?php // funds custom pamyment settings here ?>
		</tr>
		<?php
		}
		...
		{
			wp_schedule_event(time(), 'daily', 'pmpro_cron_funds_subscription_updates');
		}
		...
		{
			wp_clear_scheduled_hook('pmpro_cron_funds_subscription_updates');
		}
		 ...
		 * @since 1.8
		 */
		static function pmpro_cron_funds_subscription_updates()
		{
		}
